...
def attach_asset_features(features_df: pd.DataFrame,
    ...
    # Drop asset metadata from features_df to avoid _x/_y suffix collisions on merge
    cols_to_drop = [c for c in available if c in features_df.columns and c != "asset_id"]
...
def build_single_row_features(
    *,
    ...
    # Asset features
    feature_row["asset_age"]              = float(asset.get("age_years", asset.get("asset_age", 10)))
    feature_row["capacity_mva"]           = float(asset.get("capacity_mva", 15))
    _cust = asset.get("customers_served", None)
    if _cust is None:
        _cust = 2000.0  # neutral default — never fall back to capacity_mva
    feature_row["customers_served"]       = float(_cust)
