...
N_POSITIVE_FRAC  = 0.70        # ~70 % of assets get a failure trajectory (was 0.40)
WEATHER_SEVERE_RATE = 0.12     # 12 % of weather readings are severe (was 0.08)
...
def _degradation_ramp(n_pts: int, onset_idx: int, severity: float) -> np.ndarray:
    """Returns an array of shape (n_pts,): 0 before onset, linear ramp to `severity`."""
    arr = np.zeros(n_pts, dtype=float)
    ramp_len = n_pts - onset_idx
    if ramp_len > 1:
        arr[onset_idx:] = np.linspace(0.0, severity, ramp_len)
    return arr
...
def generate(save: bool = True) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for asset in _ASSETS:
        ...
        # Degradation parameters — onset earlier so more rows are positive
        onset_idx   = int(n_pts * float(rng.uniform(0.20, 0.55))) if is_degrading else n_pts
        severity    = float(rng.uniform(0.70, 1.0)) if is_degrading else 0.0
        ...
        if is_degrading:
            ...
            # Threshold-based labels (any point where degradation > 0.45 counts as 24h positive)
            threshold_mask = degr > 0.45
            label_24h[threshold_mask] = 1
            label_72h[degr > 0.25] = 1
        ...
        for i, ts in enumerate(timestamps):
            ...
            # Mode-specific boosts — stronger signal for clearer separation
            thermal_boost  = d * 45 if mode in ("thermal",  "combined") else 0.0
            mech_boost     = d * 12 if mode in ("mechanical","combined") else 0.0
            pd_boost       = d * 0.55 if mode in ("insulation","combined") else 0.0
            oil_drop       = d * 0.35 if mode in ("insulation","combined") else 0.0
            load_boost     = d * 25 if mode == "combined" else 0.0

            ot  = float(np.clip(base["ot"]  + diurnal + thermal_boost + rng.normal(0, 1.0), 20, 140))
            lp  = float(np.clip(base["lp"]  + diurnal * 0.5 + load_boost  + rng.normal(0, 1.5),  0, 100))
            vib = float(np.clip(base["vib"] + mech_boost + rng.normal(0, 0.12), 0, 30))
            cu  = float(np.clip(base["cu"]  + d * 3.0 + rng.normal(0, 0.25), 0, 20))
            vd  = float(np.clip(base["vd"]  + d * 2.5 + rng.normal(0, 0.18), 0, 20))
            pd_ = float(np.clip(base["pd"]  + pd_boost + rng.normal(0, 0.015), 0, 1))
            amb = float(np.clip(base["amb"] + diurnal * 0.3 + rng.normal(0, 0.4), 15, 55))
            # oil quality degrades with insulation mode
            oq  = float(np.clip(100.0 - oil_drop * 100 + rng.normal(0, 0.8), 0, 100))
