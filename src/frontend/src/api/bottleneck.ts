/**
 * Bottleneck API client — /api/bottleneck/* endpoints.
 */
...
export const gsGetAssets = (params?: { asset_type?: string; region?: string; status?: string }) =>
  gs.get<{ assets: GSAsset[]; count: number }>('/api/bottleneck/assets', { params })

export const gsGetAsset = (assetId: string) =>
  gs.get<GSAsset>(`/api/bottleneck/assets/${assetId}`)

export const gsGetTelemetry = (assetId: string, hours = 48) =>
  gs.get<{ asset_id: string; records: GSTelemetryRecord[]; count: number }>(
    `/api/bottleneck/assets/${assetId}/telemetry`, { params: { hours } }
  )

export const gsGetIncidents = (assetId: string) =>
  gs.get<{ asset_id: string; incidents: GSIncident[] }>(`/api/bottleneck/assets/${assetId}/incidents`)

export const gsGetMaintenanceHistory = (assetId: string) =>
  gs.get<{ asset_id: string; records: GSMaintenanceRecord[] }>(`/api/bottleneck/assets/${assetId}/maintenance`)

export const gsGetWeather = (assetId?: string) =>
  gs.get('/api/bottleneck/weather', { params: assetId ? { asset_id: assetId } : {} })

export const gsGetRiskRanking = (scenario?: string) =>
  gs.get<{ ranking: GSRankingEntry[]; total: number; scenario: string | null }>(
    '/api/bottleneck/risk/ranking', { params: scenario ? { scenario } : {} }
  )

export const gsGetKPIs = (scenario?: string) =>
  gs.get<GSKPIs>('/api/bottleneck/dashboard/kpis', { params: scenario ? { scenario } : {} })

export const gsGetAlerts = () =>
  gs.get<{ alerts: GSAlert[]; count: number }>('/api/bottleneck/dashboard/alerts')

export const gsGetMaintenancePriorities = (params?: {
  priority_level?: string; asset_type?: string; region?: string
}) => gs.get<{ priorities: any[]; count: number }>('/api/bottleneck/maintenance/priorities', { params })

export const gsGetCrews = (availability?: string) =>
  gs.get<{ crews: GSCrew[]; count: number }>('/api/bottleneck/crew', {
...
export const gsGetCrewPlan = () =>
  gs.get<{ assignments: GSCrewAssignment[]; standby: any[]; total_assigned: number; total_standby: number }>(
    '/api/bottleneck/crew/plan'
  )

export const gsGetAssetIntelligence = (assetId: string) =>
  gs.get<GSIntelligence>(`/api/bottleneck/assets/${assetId}/intelligence`)

export const gsSimulateScenario = (scenario: string, assetId?: string) =>
  gs.post<{ scenario: string; results: GSScenarioResult[] }>('/api/bottleneck/scenarios/simulate', {
    scenario, asset_id: assetId || null
  })

export const gsChat = (message: string, sessionId?: string) =>
  gs.post<{ session_id: string; response: string; timestamp: string }>('/api/bottleneck/chat', {
...
export const gsGetModelStatus = () =>
  gs.get<GSModelStatus>('/api/bottleneck/model/status')

export const gsGetModelMetrics = () =>
  gs.get<GSModelMetrics>('/api/bottleneck/model/metrics')

export const gsTriggerRetrain = () =>
  gs.post<{ job_id: string; status: string; message: string }>('/api/bottleneck/model/retrain', {})
